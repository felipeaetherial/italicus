...
import { useAccountsPayable } from "@/lib/hooks/use-accounts-payable";
import {
...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  embalagem: "Embalagem",
  imposto_venda: "Imposto Venda",
  comissao: "Comissao",
  salario_producao: "Salario Producao",
  salario: "Salario",
  aluguel: "Aluguel",
  energia: "Energia",
  agua: "Agua",
  manutencao: "Manutencao",
  salario_adm: "Salario Adm",
  marketing: "Marketing",
  sistema: "Sistema",
  servico_terceiro: "Servico Terceiro",
...
const COST_GROUP_LABELS: Record<string, string> = {
  variavel: "Variavel",
  fixo: "Fixo",
...
function isOverdue(item: any): boolean {
  return item.status === "pendente" && item.dueDate < todayISO();
}
...
export default function ContasPagarPage() {
  ...
  const { data, loading, error, refetch } = useAccountsPayable({
  ...
  const kpis = useMemo(() => {
    ...
    for (const item of data) {
      ...
      if (item.status === "pago" && item.paymentDate) {
        const d = new Date(item.paymentDate);
        if (d.getMonth() === currentMonth && d.getFullYear() === currentYear) {
          pagasMes += item.amount || 0;
        }
      }
    }

    return { totalPendente, vencidasCount, vencidasTotal, pagasMes };
  }, [data]);
  ...
  function handleCreate() {
    startTransition(async () => {
      const result = await createAccountPayable({
        ...
        costGroup: form.costGroup || undefined,
  ...
  function handleEdit() {
    ...
    startTransition(async () => {
      const result = await updateAccountPayable(editingItem.id, {
        description: form.description,
        supplierName: form.supplierName || undefined,
        amount: Number(form.amount),
        dueDate: form.dueDate,
  ...
  function openEdit(item: any) {
    ...
    setForm({
      ...
      costGroup: item.costGroup || "variavel",
      notes: item.notes || "",
    });
    setEditOpen(true);
  }
  ...
  function handlePay(id: string) {
    ...
    startTransition(async () => {
      const result = await payAccountPayable(id);
      if (result.success) {
        toast.success("Conta paga com sucesso");
        refetch();
      } else {
        toast.error(result.error || "Erro ao pagar");
      }
    });
  }

  function openPartialPay(item: any) {
    setEditingItem(item);
    setPartialAmount("");
    setPartialOpen(true);
  }
  ...
  function handlePartialPay() {
    if (!editingItem) return;
    startTransition(async () => {
      const result = await partialPayAccountPayable({
        id: editingItem.id,
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => {
      const result = await deleteAccountPayable(id);
      if (result.success) {
  ...
  function getStatusBadge(item: any) {
    if (item.status === "pago") {
      return <Badge className="bg-green-600 hover:bg-green-600/80">Pago</Badge>;
    }
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(v) => setForm({ ...form, costGroup: v })}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {Object.entries(COST_GROUP_LABELS).map(([key, label]) => (
              <SelectItem key={key} value={key}>
                {label}
              </SelectItem>
            ))}
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            {item.status === "pendente" && (
                              <>
                                <DropdownMenuItem onClick={() => handlePay(item.id)}>
                                  Pagar
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => openPartialPay(item)}>
                                  Pagar Parcial
                                </DropdownMenuItem>
                              </>
                            )}
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Valor total: <strong>{formatCurrency(editingItem?.amount || 0)}</strong>
            </p>
            <div>
            ...
            {partialAmount && Number(partialAmount) > 0 && (
              <p className="text-sm">
                ...
                <strong>
                  {formatCurrency(
                    Math.max(0, (editingItem?.amount || 0) - Number(partialAmount))
                  )}
                </strong>
              </p>
            )}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isPending || !partialAmount || Number(partialAmount) <= 0}
            >
              {isPending ? "Pagando..." : "Confirmar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
